...
const SongForm: React.FC<SongFormProps> = ({stlUrl, setStlUrl }) => {
  ...
  return (
    <div className='w-1/2 flex flex-col mx-auto space-y-5 px-5 border'>
      ...
      <button onClick={handleSubmit} disabled={loading} className='border rounded-md bg-gray-600 border-gray-700 p-2'>
      ...
      {stlUrl && (
        <a href={stlUrl} download='lithophane.stl'>
          <button className='border rounded-md bg-gray-600 border-gray-700 p-2'>Download STL</button>
        </a>
      )}
    </div>
  );
